...
// dep modules
var notation_1 = require("@s5rw/notation");
...
var utils = {
    ...
    validName: function (name, throwOnInvalid) {
        ...
        if (RESERVED_KEYWORDS.indexOf(name) >= 0) {
            if (!throwOnInvalid)
                return false;
            throw new core_1.AccessControlError("Cannot use reserved name: \"".concat(name, "\""));
        }
        return true;
    },
    /**
    ...
    validResourceObject: function (o) {
        ...
        utils.eachKey(o, function (action) {
            ...
            if (enums_1.actions.indexOf(s[0]) === -1) {
                throw new core_1.AccessControlError("Invalid action: \"".concat(action, "\""));
            }
            if (s[1] && enums_1.possessions.indexOf(s[1]) === -1) {
                throw new core_1.AccessControlError("Invalid action possession: \"".concat(action, "\""));
            }
            var perms = o[action];
            if (!utils.isEmptyArray(perms) && !utils.isFilledStringArray(perms)) {
                throw new core_1.AccessControlError("Invalid resource attributes for action \"".concat(action, "\"."));
            }
        });
        return true;
    },
            ...
            if (!utils.validName(resourceName, false)) {
                if (resourceName === '$extend') {
                    ...
                    if (!utils.isFilledStringArray(extRoles)) {
                        throw new core_1.AccessControlError("Invalid extend value for role \"".concat(roleName, "\": ").concat(JSON.stringify(extRoles)));
                    }
                    else {
                    ...
                    throw new core_1.AccessControlError("Cannot use reserved name \"".concat(resourceName, "\" for a resource."));
                }
            }
            else {
    ...
    getInspectedGrants: function (o) {
        ...
        if (type === 'object') {
            ...
            throw new core_1.AccessControlError("".concat(strErr, " Expected an array or object."));
        }
        return grants;
    },
    // ----------------------
    ...
    normalizeActionPossession: function (info, asString) {
        ...
        if (typeof info.action !== 'string') {
            ...
            throw new core_1.AccessControlError("Invalid action: ".concat(JSON.stringify(info)));
        }
        var s = info.action.split(':');
        if (enums_1.actions.indexOf(s[0].trim().toLowerCase()) < 0) {
            throw new core_1.AccessControlError("Invalid action: ".concat(s[0]));
        }
        ...
        if (poss) {
            if (enums_1.possessions.indexOf(poss.trim().toLowerCase()) < 0) {
                throw new core_1.AccessControlError("Invalid action possession: ".concat(poss));
            }
            else {
    ...
    normalizeQueryInfo: function (query) {
        if (utils.type(query) !== 'object') {
            throw new core_1.AccessControlError("Invalid IQueryInfo: ".concat(typeof query));
        }
        // clone the object
        ...
        if (!utils.isFilledStringArray(query.role)) {
            throw new core_1.AccessControlError("Invalid role(s): ".concat(JSON.stringify(query.role)));
        }
        // validate resource
        if (typeof query.resource !== 'string' || query.resource.trim() === '') {
            throw new core_1.AccessControlError("Invalid resource: \"".concat(query.resource, "\""));
        }
    ...
    normalizeAccessInfo: function (access, all) {
        ...
        if (utils.type(access) !== 'object') {
            throw new core_1.AccessControlError("Invalid IAccessInfo: ".concat(typeof access));
        }
        // clone the object
        ...
        if (access.role.length === 0 || !utils.isFilledStringArray(access.role)) {
            throw new core_1.AccessControlError("Invalid role(s): ".concat(JSON.stringify(access.role)));
        }
        ...
        if (access.resource.length === 0 || !utils.isFilledStringArray(access.resource)) {
            throw new core_1.AccessControlError("Invalid resource(s): ".concat(JSON.stringify(access.resource)));
        }
    ...
    getRoleHierarchyOf: function (grants, roleName, rootRole) {
        ...
        var role = grants[roleName];
        if (!role)
            throw new core_1.AccessControlError("Role not found: \"".concat(roleName, "\""));
        var arr = [roleName];
        if (!Array.isArray(role.$extend) || role.$extend.length === 0)
            return arr;
        role.$extend.forEach(function (exRoleName) {
            if (!grants[exRoleName]) {
                throw new core_1.AccessControlError("Role not found: \"".concat(grants[exRoleName], "\""));
            }
            if (exRoleName === roleName) {
                throw new core_1.AccessControlError("Cannot extend role \"".concat(roleName, "\" by itself."));
            }
            // throw if cross-inheritance and also avoid memory leak with
            // maximum call stack error
            if (rootRole && (rootRole === exRoleName)) {
                throw new core_1.AccessControlError("Cross inheritance is not allowed. Role \"".concat(exRoleName, "\" already extends \"").concat(rootRole, "\"."));
            }
    ...
    extendRole: function (grants, roles, extenderRoles) {
        ...
        if (arrExtRoles.length === 0) {
            throw new core_1.AccessControlError("Cannot inherit invalid role(s): ".concat(JSON.stringify(extenderRoles)));
        }
        var nonExistentExtRoles = utils.getNonExistentRoles(grants, arrExtRoles);
        if (nonExistentExtRoles.length > 0) {
            throw new core_1.AccessControlError("Cannot inherit non-existent role(s): \"".concat(nonExistentExtRoles.join(', '), "\""));
        }
        roles.forEach(function (roleName) {
            ...
            if (crossInherited) {
                throw new core_1.AccessControlError("Cross inheritance is not allowed. Role \"".concat(crossInherited, "\" already extends \"").concat(roleName, "\"."));
            }
    ...
    getUnionAttrsOfRoles: function (grants, query) {
        ...
        if (len > 0) {
            ...
            var i = 1;
            while (i < len) {
                attrs = notation_1.Notation.Glob.union(attrs, attrsList[i]);
                i++;
            }
        }
        return attrs;
    ...
    lockAC: function (ac) {
        ...
        /* istanbul ignore next */
        if (!locked) {
            throw new core_1.AccessControlError("Could not lock grants: ".concat(typeof _ac._grants));
        }
    ...
    filter: function (object, attributes) {
        ...
        var notation = new notation_1.Notation(object);
